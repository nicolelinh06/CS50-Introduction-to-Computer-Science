...
int get_digit_count(long long credit);
bool validate_luhn(long long credit);
...
bool validate_luhn(long long credit)
{
    ...
    while (credit > 0)
    {
        ...
        {
            int luhn_1 = digit * 2;
            total_sum += (luhn_1 > 9) ? (luhn_1 % 10) + (luhn_1 / 10) : luhn_1; // Sum the digits
        }
        else
        {
            total_sum += digit; // Sum the other digits
        }
